...
for nn=1:length(v_max_set)
    v_max=v_max_set(nn); % km/h
    for cc=1:length(KP)
...
figure(1)
semilogy(v_max_set,To(1,:),'-*b');
hold on;
semilogy(v_max_set,To(2,:),'-ob')
ax = gca;
...
filename1=strcat('ZAK-OTFS仿真结果\',string(datetime('now','Format','yyyy-MM-dd''T''HHmmss')),'_compare_v_syn-cp-3-1.fig');
